...
@Injectable()
export class CompanyRelationFilter<Entity extends CompanyAware>
  implements FilterQuery<Entity>
{
  ...
  supports(metadata: EntityMetadata): boolean {
    return (
      !!metadata.relations.find(
        (r) => r.type === COMPANY_ENTITY && r.propertyName === 'company'
      ) &&
  ...
  execute(qb: SelectQueryBuilder<Entity>): void {
    qb.leftJoin(`${qb.alias}.company`, 'company').andWhere('company.id = :id', {
      id: RequestContextService.getCurrentUser().company.id,
    });
  }
}
