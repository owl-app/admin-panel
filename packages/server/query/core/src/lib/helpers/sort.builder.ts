...
export class SortBuilder {
  static build<DTO>(sorts: SortField<DTO>[]): Sorter<DTO> {
    const comparators = sorts.map(({ field, direction, nulls }) =>
      this.buildComparator(field, direction, nulls)
    );
  ...
  static nullsComparator<Field>(
    direction: SortDirection,
    nulls?: SortNulls
  ): SortComparator<Field> {
    switch (nulls) {
