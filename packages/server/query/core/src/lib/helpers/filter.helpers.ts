...
export const getFilterFieldComparison = <DTO, K extends keyof FilterComparisons<DTO>>(
  ...
  obj[field] as FilterFieldComparison<DTO[K]> & Filter<DTO[K]>;
...
export const getFilterFields = <DTO>(filter: Filter<DTO>): string[] => {
  const fieldSet: Set<string> = Object.keys(filter).reduce(
    (fields: Set<string>, filterField: string): Set<string> => {
      if (filterField === 'and' || filterField === 'or') {
...
export const getFilterOmitting = <DTO>(
  filter: Filter<DTO>,
  ...keys: (keyof Filter<DTO>)[]
): Filter<DTO> =>
