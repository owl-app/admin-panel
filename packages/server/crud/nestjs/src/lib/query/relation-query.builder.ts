...
import { AggregateQuery, Class, Query } from '@owl-app/crud-core'
...
export class RelationQueryBuilder<Entity, Relation> {
  ...
  private getManyToOneOrOneToOneOwnerMeta(relation: RelationMetadata): RelationQuery<Relation, Entity> {
    ...
    return {
      ...
      mapRelations: <RawRelation>(entity: Entity, relations: Relation[], rawRelations: RawRelation[]): Relation[] => {
        ...
        return filteredentityRelations.reduce((entityRelations: Array<RawRelation|Relation>, rawRelation: RawRelation | Relation) => {
  ...
  private batchMapRelationsManyToMany<RawRelation>(
    ...
    return filteRawRelations.reduce((entityRelations: Array<RawRelation|Relation>, rawRelation: RawRelation | Relation) => {
      const filter = this.getRelationPrimaryKeysPropertyNameAndColumnsName().reduce(
        (columnsFilter, column) => ({
          ...columnsFilter,
          [column.propertyName]: rawRelation[column.columnName as keyof typeof rawRelation]
  ...
  private buildAlias(...alias: string[]): string {
    return DriverUtils.buildAlias(this.relationRepo.manager.connection.driver, this.relationMeta.fromAlias as unknown, ...alias)
  }
}
