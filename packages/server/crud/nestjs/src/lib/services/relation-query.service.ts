...
export abstract class RelationQueryService<Entity> {
  ...
  public async findRelation<Relation>(
    ...
    const assembler = AssemblerFactory.getAssembler(RelationClass, this.getRelationEntity(relationName))
    let relationEntity = opts?.lookedAhead ? dto[relationName as keyof typeof dto] as unknown : undefined
  ...
  async batchAggregateRelations<Relation>(
    ...
    return rawAggregates.reduce((results, relationAgg) => {
      ...
      results.set(e, [
        ...resultingAgg,
        ...AggregateBuilder.convertToAggregateResponse([omit(relationAgg, relationQueryBuilder.entityIndexColName)])
      ])
      return results
   ...
   */
  async batchFindRelations<Relation>(
    RelationClass: Class<Relation>,
    relationName: string,
    dtos: Entity[],
    opts?: FindRelationOptions<Relation>
    ...
    if (opts?.lookedAhead) {
      const {isNullable} = this.getRelationMeta(relationName)

      // Make sure the data is there
      if (
        (isNullable && dtos.some((entity) => entity[relationName  as keyof typeof entity])) ||
        (!isNullable && dtos.some((entity) => entity[relationName  as keyof typeof entity]))
      ) {
        const assembler = AssemblerFactory.getAssembler(RelationClass, this.getRelationEntity(relationName))

        return dtos.reduce((results, entity) => results.set(entity, entity[relationName  as keyof typeof entity] ? assembler.convertToDTO(entity[relationName  as keyof typeof entity]) : undefined), new Map<Entity, Relation>())
      }
    }
...
results.set(dto, relation[0])
  ...
  createTypeormRelationQueryBuilder(entity: Entity, relationName: string): TypeOrmRelationQueryBuilder<Entity> {
  ...
  getRelations<Relation>(relationName: string, ids: (string | number)[], filter?: Filter<Relation>): Promise<Relation[]> {
